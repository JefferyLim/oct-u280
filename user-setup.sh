...
if [ -f "$SENTINEL" ]; then
    echo "User script already ran; exiting."
    exit 0
fi
...
install_custom_lib(){
    ...
    git clone -b fix-vf_counts https://github.com/JefferyLim/OS4C /users/jlim/OS4C
...
export PATH="$PATH:/user/jlim/OS4C/fpga/lib/pcie/scripts:/user/jlim/oct-helper/fpga/:/user/jlim/oct-helper/network"
EOF
